...
public class StorageNode extends Node implements TelemetryProperties {
    ...
    @AzureOperation(name = "user/storage.open_portal.account", params = {"this.storageAccount.name()"}, type = AzureOperation.Type.ACTION)
    private void openInPortal() {
        openResourcesInPortal(this.storageAccount.getSubscriptionId(), storageAccount.id());
    }

    @AzureOperation(name = "user/storage.delete_account.account", params = {"this.storageAccount.name()"}, type = AzureOperation.Type.ACTION)
    private void delete() {

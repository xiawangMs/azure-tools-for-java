...
public class StorageNode extends Node implements TelemetryProperties {
    ...
    @AzureOperation(name = "action/storage.open_portal.account", params = {"this.storageAccount.name()"}, type = AzureOperation.Type.ACTION)
    private void openInPortal() {
        openResourcesInPortal(this.storageAccount.getSubscriptionId(), storageAccount.id());
    }

    @AzureOperation(name = "action/storage.delete_account.account", params = {"this.storageAccount.name()"}, type = AzureOperation.Type.ACTION)
    private void delete() {

...
public class RedisCacheNode extends Node implements TelemetryProperties {
    ...
    @AzureOperation(name = "user/redis.delete_redis.redis", params = {"this.name"}, type = AzureOperation.Type.ACTION)
    private void delete() {
        this.getParent().removeNode(this.subscriptionId, this.resourceId, this);
    }

    @AzureOperation(name = "user/redis.open_portal.redis", params = {"this.name"}, type = AzureOperation.Type.ACTION)
    ...
    @AzureOperation(name = "user/redis.show_properties.redis", params = {"this.name"}, type = AzureOperation.Type.ACTION)
    private void showProperties() {
        DefaultLoader.getUIHelper().openRedisPropertyView(this);
    }

    @AzureOperation(name = "user/redis.open_redis_explorer.redis", params = {"this.name"}, type = AzureOperation.Type.ACTION)

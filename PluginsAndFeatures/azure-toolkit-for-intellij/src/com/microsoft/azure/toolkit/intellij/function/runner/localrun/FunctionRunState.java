...
//            final ConfigurationFactory configFactory = RemoteDebugConfiguration RemoteConfigurationType.getInstance().getConfigurationFactories()[0];
//            final RemoteDebugConfiguration remoteConfig = new RemoteConfiguration(project, configFactory);
...
//    }
    ...
    private boolean initVENVAndRequirements(CLIExecutor executor, RunProcessHandler processHandler) {

        String python = CommandUtils.isWindows() ? "py" : "python3";

        String[] installCommand = new String[] {python, "-m", "pip", "install", "-r", "requirements.txt"};

        String[] command = FunctionUtils.commandWithVENV(installCommand, executor.localProject);

        try {
            executor.execCommand(command, project.getBasePath(), "Successfully installed requirements",
                    "Could not install requirements", true, processHandler);
        }
    ...
    private void runFunctionCli(RunProcessHandler processHandler, File stagingFolder)
        ...
        final int debugPort = FunctionUtils.findFreePortForApi(DEFAULT_DEBUG_PORT);
        final int funcPort = FunctionUtils.findFreePortForApi(Math.max(DEFAULT_FUNC_PORT, debugPort + 1));

        CLIExecutor executor = new CLIExecutor(project);

        if(!initVENVAndRequirements(executor, processHandler)) {
            //TODO: Inform the user that something failed, log error
        }

        String[] startCommand = new String[]{functionRunConfiguration.getFuncPath(), "host", "start", "-p", String.valueOf(funcPort)};

        String[] command = FunctionUtils.commandWithVENV(startCommand, project);

        processHandler.println(message("function.run.hint.port", funcPort), ProcessOutputTypes.SYSTEM);

        List<String> funcNames = functionRunConfiguration.getFunctionsToRun();
        if(funcNames.size() != FunctionUtils.findFunctionsByAnnotation(project).size()) {
            String[] funcNamesArray = funcNames.toArray(new String[0]);
            command = ArrayUtils.addAll(command, "--functions");
            command = ArrayUtils.addAll(command, funcNamesArray);
        }

        try {
            executor.execCommand(command, project.getBasePath(), functionRunConfiguration.getModuleName() + " function(s) exited gracefully",
                    functionRunConfiguration.getModuleName() + " process terminated", false, processHandler);
            if(isDebugMode()) {
                //TODO: Launch remote debugger.
            }

...
public class LanguageServerDefinition {
    ...
    public void stopAll() {
        try {
            streamConnectionProviders.values().forEach(StreamConnectionProvider::stop);
            streamConnectionProviders.clear();
        } catch (Throwable e) {
            LOG.error("Error while stopping all language servers", e);
        }
    }

...
public class IntelliJCosmosActionsContributor implements IActionsContributor {
    ...
    @AzureOperation(name = "action/cosmos.open_database_tools.account", params = {"account.getName()"}, type = AzureOperation.Type.ACTION)
    ...
    private <T extends AzResource> void openResourceConnector(@Nonnull final T resource, @Nonnull final AzureServiceResource.Definition<T> definition, Project project) {
        final Function<AzResource, AzureString> titleSupplier = r -> OperationBundle.description("action/resource.connect_resource.resource", r.getName());

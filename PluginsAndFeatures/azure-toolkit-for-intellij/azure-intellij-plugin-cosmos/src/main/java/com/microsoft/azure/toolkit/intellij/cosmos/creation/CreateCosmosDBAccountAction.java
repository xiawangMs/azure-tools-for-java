...
public class CreateCosmosDBAccountAction {
    ...
    @AzureOperation(name = "action/cosmos.create_account.account", params = {"config.getName()"}, type = AzureOperation.Type.ACTION)
    private static void doCreate(final CosmosDBAccountDraft.Config config, final Project project) {
        final AzureString title = OperationBundle.description("action/cosmos.create_account.account", config.getName());

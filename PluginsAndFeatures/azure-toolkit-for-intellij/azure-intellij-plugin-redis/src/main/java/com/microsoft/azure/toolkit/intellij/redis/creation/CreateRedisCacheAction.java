...
public class CreateRedisCacheAction {
    ...
    @AzureOperation(name = "action/redis.create_redis.redis", params = {"config.getName()"}, type = AzureOperation.Type.ACTION)
    private static void doCreate(final RedisConfig config, final Project project) {
        final AzureString title = OperationBundle.description("action/redis.create_redis.redis", config.getName());

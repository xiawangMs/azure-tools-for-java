...
public class SettingPanel extends AzureSettingPanel<WebAppOnLinuxDeployConfiguration> implements WebAppOnLinuxDeployView {
    ...
    private void init() {
        // init docker configuration panel
        containerSettingPanel = new ContainerSettingPanel(project);
        pnlAcr.add(containerSettingPanel.getPnlRoot(), new GridConstraints(0, 0, 1, 1, 0, GridConstraints.FILL_BOTH, 3, 3, null, null, null, 0));
    ...
    public void apply(WebAppOnLinuxDeployConfiguration webAppOnLinuxDeployConfiguration) {
        ...
        secureStore.savePassword(com.microsoft.azure.toolkit.intellij.containerregistry.pushimage.ui.SettingPanel.PRIVATE_DOCKER_REGISTRY, containerSettingPanel.getServerUrl(), containerSettingPanel.getUserName(),
                containerSettingPanel.getPassword());
    ...
    public void resetFromConfig(@NotNull WebAppOnLinuxDeployConfiguration conf) {
        ...
        if (StringUtils.isNotEmpty(acrInfo.getServerUrl())) {
            final ISecureStore secureStore = AzureStoreManager.getInstance().getSecureStore();
            secureStore.migratePassword(acrInfo.getServerUrl(), acrInfo.getUsername(), com.microsoft.azure.toolkit.intellij.containerregistry.pushimage.ui.SettingPanel.PRIVATE_DOCKER_REGISTRY, acrInfo.getServerUrl(), acrInfo.getUsername());
            acrInfo.setPassword(secureStore.loadPassword(com.microsoft.azure.toolkit.intellij.containerregistry.pushimage.ui.SettingPanel.PRIVATE_DOCKER_REGISTRY, acrInfo.getServerUrl(), acrInfo.getUsername()));
        }

...
public class CreateSpringCloudAppAction {
    ...
    @AzureOperation(name = "action/springcloud.create_app.app", params = "config.getAppName()", type = AzureOperation.Type.ACTION)
    private static void createApp(SpringCloudAppConfig config) {
        AzureTaskManager.getInstance().runInBackground(OperationBundle.description("action/springcloud.create_app.app", config.getAppName()), () -> {

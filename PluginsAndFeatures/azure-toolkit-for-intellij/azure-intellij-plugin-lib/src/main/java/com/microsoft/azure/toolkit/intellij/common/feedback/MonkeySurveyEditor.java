...
public class MonkeySurveyEditor extends BaseEditor implements DumbAware {
    ...
    public MonkeySurveyEditor(final Project project, VirtualFile virtualFile) {
        super(virtualFile);
        ...
        final String url = Objects.requireNonNull(virtualFile.getUserData(MonkeySurvey.SURVEY_URL));
        final Integer score = Objects.requireNonNull(virtualFile.getUserData(MonkeySurvey.ORIGINAL_CORE));
        ...
        myJSQueryOpenInBrowser.addHandler((e) -> {
            AzureTaskManager.getInstance().runLater(this::closeEditor);
            return new JBCefJSQuery.Response("ok!");
        });
        client.addRequestHandler(openLinkWithLocalBrowser(), browser);
        client.addLoadHandler(modifySubmitButtonInSurveyCollectionPage(score), browser);
        pnlBrowser.setVisible(false);
    }

    @AzureOperation(name = "action/feedback.close_monkey_survey", type = AzureOperation.Type.ACTION)
    ...
    @Nonnull
    private CefLoadHandlerAdapter modifySubmitButtonInSurveyCollectionPage(int score) {
        return new CefLoadHandlerAdapter() {
            @Override
            public void onLoadEnd(CefBrowser browser, CefFrame frame, int httpStatusCode) {
                MonkeySurveyEditor.this.pnlLoading.setVisible(false);
                MonkeySurveyEditor.this.pnlBrowser.setVisible(true);
                if (!browser.getURL().contains("www.surveymonkey.com/r/PNB5NBL")) {
                    return;
                }
                String js = "window.JavaPanelBridge = {\n" +
                    "    closeTab: function (e) {\n" +
                    ...
                    "}";
                if (score > 0 && score <= 5) {
                    js += "var starIcon = document.querySelectorAll('.emoji-rating:not(.selected) > .emoji-color.star')[" + (score - 1) + "];\n" +
                        "if (starIcon) {\n" +
                        "    starIcon.click();" +
                        "}";
    ...
    private CefRequestHandlerAdapter openLinkWithLocalBrowser() {
        return new CefRequestHandlerAdapter() {
            @Override
            @AzureOperation(name = "action/feedback.open_link_from_survey", type = AzureOperation.Type.ACTION)

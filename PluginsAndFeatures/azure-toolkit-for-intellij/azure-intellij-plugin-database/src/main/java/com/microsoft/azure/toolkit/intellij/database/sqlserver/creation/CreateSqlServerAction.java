...
public class CreateSqlServerAction {
    ...
    @AzureOperation(name = "action/sqlserver.create_server.server", params = {"config.getName()"}, type = AzureOperation.Type.ACTION)
    private static void doCreate(final DatabaseServerConfig config, final Project project) {
        final AzureString title = OperationBundle.description("action/sqlserver.create_server.server", config.getName());

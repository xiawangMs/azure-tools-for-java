...
public class CreateKubernetesServiceAction {
    ...
    @AzureOperation(name = "action/kubernetes.create_cluster.cluster", params = {"config.getName()"}, type = AzureOperation.Type.ACTION)
    private static void doCreate(final KubernetesClusterDraft.Config config, final Project project) {
        final AzureString title = OperationBundle.description("action/kubernetes.create_cluster.cluster", config.getName());

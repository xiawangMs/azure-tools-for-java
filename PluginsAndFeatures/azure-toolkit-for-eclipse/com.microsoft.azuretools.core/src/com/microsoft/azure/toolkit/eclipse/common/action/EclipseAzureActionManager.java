...
public class EclipseAzureActionManager extends AzureActionManager {
	...
	private static final ICommandService cmdService = (ICommandService) PlatformUI.getWorkbench()
			.getService(ICommandService.class);
	private static final IHandlerService handlerService = (IHandlerService) PlatformUI.getWorkbench()
	...
	public static void register() {
		...
		Arrays.stream(configurationElements).map(element -> {
			try {
				...
				.sorted(Comparator.comparing(IActionsContributor::getOrder)).forEachOrdered(contributor -> {
	...
	public <D> Action<D> getAction(Id<D> id) {
		...
		return Optional.ofNullable((Action<D>) actions.get(actionId)).orElseGet(() -> {
			return new Action<>(id).withHandler((D d, ExecutionEvent event) -> {
				try {
